...
class HistoryDetailsPageDetails extends StatefulWidget {
  final ServerModel server;
  final HistoryModel history;
  final Color? titleColor;
  final Widget loadingIndicator;

  const HistoryDetailsPageDetails({
    super.key,
  ...
  @override
  State<HistoryDetailsPageDetails> createState() => _HistoryDetailsPageDetailsState();
}

class _HistoryDetailsPageDetailsState extends State<HistoryDetailsPageDetails> {
  @override
  void initState() {
    super.initState();
    ...
    if (widget.history.ipAddress != null) {
      context.read<GeoIpBloc>().add(
        GeoIpFetched(server: widget.server, ipAddress: widget.history.ipAddress!),
      );
    }
  }
          ...
          child: Column(
            children: [
              _ItemRow(
                ...
                item: Text(widget.history.friendlyName ?? '').sensitive(),
                titleColor: widget.titleColor,
              ),
              _ItemRow(
                title: LocaleKeys.platform_title.tr(),
                item: Text(widget.history.platform ?? ''),
                titleColor: widget.titleColor,
              ),
              _ItemRow(
                title: LocaleKeys.product_title.tr(),
                item: Text(widget.history.product ?? ''),
                titleColor: widget.titleColor,
              ),
              _ItemRow(
                title: LocaleKeys.player_title.tr(),
                item: Text(widget.history.player ?? ''),
                titleColor: widget.titleColor,
              ),
              _ItemRow(
                title: LocaleKeys.decision_title.tr(),
                item: Text(StringHelper.mapStreamDecisionToString(widget.history.transcodeDecision)),
                titleColor: widget.titleColor,
              ),
              _ItemRow(
                title: LocaleKeys.ip_address_title.tr(),
                item: Text(widget.history.ipAddress ?? '').sensitive(),
                titleColor: widget.titleColor,
              ),
                ...
                _ItemRow(
                  ...
                  item: BlocBuilder<GeoIpBloc, GeoIpState>(
                    builder: (context, geoIpState) {
                      final GeoIpModel? geoIpItem = geoIpState.geoIpMap[widget.history.ipAddress];
              ...
              if (widget.history.pausedCounter != null)
                _ItemRow(
                  ...
                  item: Text(TimeHelper.simple(widget.history.pausedCounter!)),
                  titleColor: widget.titleColor,
                ),
              if (widget.history.duration != null)
                _ItemRow(
                  title: LocaleKeys.duration_title.tr(),
                  item: Text(TimeHelper.simple(widget.history.duration!)),
                  titleColor: widget.titleColor,
                ),
              _ItemRow(
                title: LocaleKeys.progress_title.tr(),
                item: Text('${widget.history.percentComplete}%'),
                titleColor: widget.titleColor,
              ),
            ],
          ),
